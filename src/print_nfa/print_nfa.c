...
static int is_visited(struct state *visited[MAX_STATE_COUNT],
                      const struct state *item, int visitor_count) {
...
static int get_ind(struct state *visited[MAX_STATE_COUNT],
                   const struct state *item, int visitor_count) {
  for (int i = 0; i < visitor_count; i++) {
