...
static struct fragment *create_fragment() {
  ...
  frag->start = (struct state){.t1 = {.to = NULL}, .t2 = {.to = NULL}};
  frag->end = (struct state){.t1 = {.to = NULL}, .t2 = {.to = NULL}};
  return frag;
}
    ...
    if (t.type == T_LIT) {  // frag = 'a'
      ...
      switch (t.type) {
        case T_CAT: {  // frag = lhs.rhs
          ...
          if (!rhs || !lhs) {
            sprintf(error,
                    "thompson: could not retrieve fragments for concatenation");
            free_nfa(nfa);
            free(stack);
            return -1;
          }
        ...
        case T_OR: {  // frag = lhs|rhs
          ...
          frag->start = (struct state){.t1 = {.sym = EPS, .to = &lhs->start},
        ...
        case T_STAR: {  // frag = prev*
          ...
          if (!prev) {
            sprintf(error,
                    "thompson: could not retrieve fragment for kleene star");
            free_nfa(nfa);
            free(stack);
            return -1;
          }
          ...
          frag->start = (struct state){.t1 = {.sym = EPS, .to = &prev->start},
                                       .t2 = {.sym = EPS, .to = &frag->end}};

          prev->end = (struct state){.t1 = {.sym = EPS, .to = &prev->start},
                                     .t2 = {.sym = EPS, .to = &frag->end}};
          nfa->frags[nfa->frag_count++] = frag;
          stack_push(stack, frag);
